	...
	return (
		<div className="collabs-list">
			{items.map((item) => {
				...
				let isInCombo;
				let isFirstItemDisabled;
				let isSecondItemDisabled;
				let isCollabItemDisabled;

				if (comboMode) {
					isInCombo = comboItems.includes(item);
					isFirstItemDisabled =
						firstItem && !!comboItems.find((ci) => ci.requires!.includes(firstItem.id));
					isSecondItemDisabled =
						secondItem && !!comboItems.find((ci) => ci.requires!.includes(secondItem.id));
				...
				return (
					<React.Fragment key={item.name}>
						<Sprite
							...
							onSelected={
								comboMode && isFirstItemDisabled ? undefined : () => onItemClicked(firstItem!)
							}
						/>
						...
						<Sprite
							...
							onSelected={
								comboMode && isSecondItemDisabled ? undefined : () => onItemClicked(secondItem!)
							}
						/>
						...
						<Sprite
							...
							selected={comboMode ? comboItems.includes(item) : item === selectedItem}
							onSelected={comboMode && isCollabItemDisabled ? undefined : handleCollabItemClicked}
						/>
					</React.Fragment>
				);
			})}
